...
// 'home' is the nav key for the About section ("À Propos" / "من نحن")
const NAV_LINKS: Array<{ key: 'home' | 'products' | 'services' | 'portfolio' | 'branches' | 'contact'; hash: string }> = [
  { key: 'home', hash: 'about' },
              ...
              <ul className="flex flex-col gap-3">
                {NAV_LINKS.map(({ key, hash }) => (
                  <li key={key}>
                    <Link
                      href={`/${locale}#${hash}`}
                      className="font-sans text-sm text-brand-300/60 hover:text-white transition-colors duration-200 ltr:hover:translate-x-0.5 rtl:hover:-translate-x-0.5 inline-block"
                    >
                      {tNav(key)}
                    </Link>
                  </li>
