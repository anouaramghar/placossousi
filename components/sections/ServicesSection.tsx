      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="reveal mb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mt-5">
            <h2 className="font-display text-transparent bg-clip-text bg-gradient-to-b from-white to-brand-200/50 text-[clamp(2.8rem,7vw,5.5rem)] tracking-[-0.04em] leading-[1.04] text-glow-soft">
              {t('title')}
            </h2>
            <p className="font-sans text-brand-300/50 text-sm tracking-[0.12em] font-medium mt-4 md:mt-0 md:mb-3">{items.length} {t('label').toLowerCase()}</p>
          </div>
        </div>
          ...
          {items.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Index number */}
                <span className="font-display text-brand-800 text-5xl md:text-7xl tabular-nums min-w-[70px] md:min-w-[100px] text-right select-none group-hover:text-brand-700/70 transition-colors duration-500 relative z-10 leading-none">
                  {String(i + 1).padStart(2, '0')}
                </span>

                {/* Icon box */}
                <div className="w-14 h-14 md:w-18 md:h-18 bg-gradient-to-br from-brand-700/80 to-brand-900 rounded-2xl flex items-center justify-center border border-white/5 flex-shrink-0 group-hover:scale-110 group-hover:rotate-6 group-hover:border-brand-400/30 transition-all duration-500 relative z-10 shadow-[0_4px_16px_rgba(0,0,0,0.4)]">
                  ...
                  <IconComponent className="w-6 h-6 md:w-8 md:h-8 text-brand-300 group-hover:text-white relative z-10 transition-colors duration-300" strokeWidth={1.5} />
                </div>
